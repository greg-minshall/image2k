...
if (!is.na(get_Rscript_filename())) {
  ...
  parserun(
    c(
      strsplit(
        sprintf("-f %s --pref.lib 2 --suffix %s -v", file, suffix2),
        split=" ")[[1]]));
  parserun(
    c(
      strsplit(
        sprintf("-f %s --pref.lib k --suffix %s -v", file, suffixk),
        split=" ")[[1]]));
  ...
  status <- 0;
  for (i in c("2", "k")) {
    x <- read.image2k(file, pref.lib=i);
    write.image2k(file=outfile, pm=x, pref.lib=i);
    y <- read.image2k(file, pref.lib=i);
    if (!identical(x,y)) {
